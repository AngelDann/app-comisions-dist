...
2. **Run automated checks** (use the project's toolchain; prefer `uv run` if `uv` is configured, otherwise activate the venv and run):
   ```bash
...
Adapted from [claude-code-django](https://github.com/kjnez/claude-code-django) (`.claude/skills/code-quality`).
