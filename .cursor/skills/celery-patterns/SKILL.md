...
If this project does not use `uv`, run the same commands with your virtualenv activated (e.g. `celery -A <project> worker -l info`). Adjust `-A` to your Celery app path (`fito_ai` / `config` as applicable).

## Source

Adapted from [claude-code-django](https://github.com/kjnez/claude-code-django) (`.claude/skills/celery-patterns`).
